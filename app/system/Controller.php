...
class Controller extends App
{
    ...
    private function setPath($render)
    {
        if(is_array($render))
        {
            foreach ($render as $li) {
                $path = 'app/views'. $this->getArea() . '/' . $this->getController() . '/'  . $li . '.phtml';
                $this->fileExists($path);
                $this->path[] = $path;
            }
        }else{
            $this->pathRender = is_null($render) ? $this->getAction() : $render;
            $this->path = 'app/views/'.$this->getArea().'/'.$this->getController().'/'.$this->pathRender.'.phtml';
    ...
    private function fileExists($file)
    {
        if (!file_exists($file))
        {
            ...
            define('APP_ERROR', 'Could not find file '.$file);
    ...
    public function view($render = null)
    {
        ...
        if(is_null($this->layout))
        {
            ...
            $this->layout = 'app/templates/'.$this->getArea().'/layouts/'.$this->layout.'.phtml';
            if(file_exists($this->layout))
            {
                $this->render($this->layout);
            }else{
                header('HTTP/1.0 404 Not Found');
                define('APP_ERROR', 'Could not find the layout');
                include("./app/templates/system/error_page.phtml");
                die();
            }
        }
