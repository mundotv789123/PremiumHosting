...
class App extends Routers
{
    ...
    {
        $this->url = isset($_GET['url']) ? trim(strip_tags(addslashes($_GET['url']))) : 'home/index';
    }
    ...
    private function validateController()
    {
        if(!(class_exists($this->runController)))
        {
            ...
            define('APP_ERROR', 'Página "'.$this->controller.'" não encontrada');
            include("./app/templates/{$this->getArea()}/layouts/error.phtml");
            die();
        }
    }
    ...
    private function validateAction()
    {
        ...
        {
            header('HTTP/1.0 404 Not Found');
            define('APP_ERROR', 'Could not find action '.$this->action);
    ...
    {
        $this->runController = 'app\\controllers\\' . $this->getArea() . '\\'. $this->controller . 'Controller';
